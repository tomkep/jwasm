...
/* set the global variables Frame and Frame_Datum */
{
...
*/
{
    ...
    if( fixup != NULL ) {
        ...
        if( fixup->fixup_type != FIX_SEG ) {
            CodeInfo->data[index] += fixup->offset;
        }
        /*
...
*/
{
    ...
    if( sym->state == SYM_UNDEFINED ) {
        ...
        if( sym->state == SYM_EXTERNAL ) {
            ...
            fixnode->lr.target = TARGET_EXT;
            fixnode->lr.target_datum = ((dir_node *)sym)->e.procinfo->idx;
        } else {
            /* it's most likely a SYM_INTERNAL */
