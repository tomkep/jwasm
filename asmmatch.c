...
/******************************/
{
    ...
    if( ins->byte1_info == F_0F0F ) {
        AsmCodeByte( ins->opcode | CodeInfo->info.opcode );
    }
    return( NOT_ERROR );
}
...
*/
{
    ...
    while( out > 0 ) {
        AsmCodeByte( CodeInfo->data[index] );
...
//    DebugMsg(("match_phase_1 enter\n"));
    ...
    // this hack makes the CALLF/JMPF hack obsolete
    if (CodeInfo->isfar) {
        if (CodeInfo->info.token == T_CALL)
            i = i + 8; /* 8 is no of entries for CALLN in ASMINSD.H */
        else if (CodeInfo->info.token == T_JMP)
            i = i + 9; /* 9 is no of entries for JMPN in ASMINSD.H */
    }
